/* 雙北行政區內AED自動體外心臟去顫器數量 */
SELECT 場所區域 AS x_axis, aed_count AS data
FROM aed_count_by_region
WHERE 場所縣市 = '臺北市' OR 場所縣市 = '新北市'
GROUP BY 場所區域, aed_count
...
/* 臺北市行政區內AED自動體外心臟去顫器數量 */
SELECT 場所區域 AS x_axis, aed_count AS data
FROM aed_count_by_region_tp
WHERE 場所縣市 = '臺北市'
GROUP BY 場所區域, aed_count
ORDER BY ARRAY_POSITION(
  ARRAY[
